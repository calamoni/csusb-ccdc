#!/bin/sh
# Script to compare current system configurations with the latest backup
# Usage: diff_configs.sh [system_name] [backup_directory]
...
case "$SYSTEM_NAME" in
    "all")
        ...
        if [ -f "${latest_backup}/binaries.md5" ]; then
            ...
            grep -vf "${latest_backup}/binaries.md5" "$TEMP_DIR/current_binaries.md5" || echo "No checksum differences found"
            echo "New binary files (not in backup):"
            grep -vf "$TEMP_DIR/current_binaries.md5" "${latest_backup}/binaries.md5" || echo "No new binary files found"
        else
